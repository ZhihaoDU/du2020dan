...
def get_speech_slices(label, feat, noise, frame_number):
    ...
    noisy_slices = torch.zeros(bb * 4, 40, dd).to(label)
    noise_slices = torch.zeros(bb * 4, 40, dd).to(label)
    for i in range(bb):
        ...
        for j in range(4):
            ...
            noisy_slices[i * 4 + j, :, :] = feat[i, ss: ss + 40, :]
            noise_slices[i * 4 + j, :, :] = noise[i, ss: ss + 40, :]
    return speech_slices, noisy_slices, noise_slices
...
if __name__ == '__main__':
    ...
    parser.add_argument('--TTUR', type=bool, default=False)
    ...
    model_name_list = [args.feature_domain, args.adversarial_loss, args.model_config, args.target_type]
    ...
    if args.TTUR:
        d_optimizer = optim.Adam(discriminator.parameters(), lr=4e-4, betas=(0., 0.9))
    else:
    ...
    while epoch != args.total_epoch:
        ...
        for iteration, (clean_frame, noisy_frame, frame_number, batch_mask) in enumerate(train_dataloader):
            ...
            with torch.no_grad():
                label_slices, feat_slices, noise_slices = get_speech_slices(label, feat, noise, frame_number)

                results = calc_target(feat_slices, label_slices, noise_slices, None, opts)
            ...
            disc_g_loss_dict = disc_g_loss(discriminator, (fake_data,), backward=True, opts=dis_opts)
            g_optimizer.step()
            g_loss = disc_g_loss_dict["g_loss"]
        ...
        with torch.no_grad():
            ...
            generator.eval()
            discriminator.eval()
            plot_x = generator.forward((plot_z,))
