...
def get_speech_slices(label, results, frame_number):
    ...
    label_slices = (label_slices - real_data_min) / (real_data_max - real_data_min)
    label_slices = F.interpolate(label_slices * 2. - 1., 64, 64)
    for key in slices:
        if key == "real_data" or key == "fake_data":
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_config', type=str, default='BiFreqMelCRN16c2l1024u')
    ...
    model_name_list = ["valid_no_aug", args.feature_domain, args.model_config]
    adversarial_loss = args.adversarial_loss
    reconstruction_loss = args.reconstruction_loss
    if adversarial_loss:
        dis_model = model_opts['dis_model_name']
        model_name_list.append(str(dist_alpha))
    ...
    if adversarial_loss:
        ...
        if args.disc_name is not None and args.disc_step > 0:
    ...
    for epoch in range(args.total_epoch):
        if epoch > 0 or train_first:
            ...
            for iteration, (clean_frame, noisy_frame, frame_number, batch_mask) in enumerate(train_dataloader):
                    ...
                    if adversarial_loss:
                        if args.d_iter > 0:
                            ...
                            writer.add_scalar('train/d_fake_loss', disc_d_loss_dict["fake_loss"].item(),
                                              global_step // args.log_interval)
                        writer.add_scalar('train/g_loss', disc_g_loss_dict["g_loss"].item() * dist_alpha,
                                          global_step // args.log_interval)
                if global_step % args.summary_interval == 0:
                    ...
                    if adversarial_loss is not None:
                        add_tf_image(writer, real_data, "train/real_data", 64, 8,
                                     global_step // args.summary_interval)
                        add_tf_image(writer, fake_data, "train/fake_data", 64, 8,
                                     global_step // args.summary_interval)
        ...
        with torch.no_grad():
            ...
            for iteration, (clean_frame, noisy_frame, frame_number, batch_mask) in enumerate(valid_dataloader):
                ...
                if adversarial_loss is not None:
                    label_slices, real_data, fake_data = get_speech_slices(label, results, frame_number)
                    disc_g_loss_dict = disc_g_loss(discriminator, (fake_data,), backward=False,
                                                   opts=dis_opts)
                    valid_loss[1] += disc_g_loss_dict["g_loss"].item() * dist_alpha
                recon_loss = reconstruction_loss(predict, target, batch_mask, backward=False, opts=None)["loss"] / BATCH_SIZE
        ...
        if adversarial_loss:
            writer.add_scalar('valid/g_loss', valid_loss[1], epoch)

        # Save the best model to step 0
        if valid_loss[2] < lowest_valid_loss:
            logger.info("Save best models at step %d" % global_step)
            lowest_valid_loss = valid_loss[2]
            if adversarial_loss:
