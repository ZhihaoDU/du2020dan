...
def d_loss(discriminator, real_batch, fake_batch, backward=False, opts=None):
    ...
    real_loss = ((logits_real - fake_mean) - 1.).pow(2.).mean()
    fake_loss = ((logits_fake - real_mean) + 1.).pow(2.).mean()
    if backward:
...
def g_loss(discriminator, real_batch, fake_batch, backward=False, opts=None):
    logits_real, _ = discriminator.forward(real_batch)
    logits_fake, feat = discriminator.forward(fake_batch)
    ...
    if backward:
        g_loss.backward()
    return {"g_loss": g_loss, "logits_mean": fake_mean, "feat": feat}
