...
def get_speech_slices(label, results, frame_number):
    ...
    slices = {}
    for key in results:
        if key == "real_data" or key == "fake_data" or key == "condition":
    ...
    real_data = torch.cat([slices['real_data'], slices["condition"]], dim=1)
    fake_data = torch.cat([slices['fake_data'], slices["condition"]], dim=1)
    return label_slices, real_data, fake_data
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_config', type=str, default='BiFreqMelCRN_DCGAN2C')
    ...
    parser.add_argument('--adversarial_loss', type=str, default="ralsgan_gp")
    ...
    model_name_list = [args.feature_domain, args.reconstruction_loss, args.adversarial_loss, args.model_config]
    ...
    train_dataset = FrameDataset([-6, -3, 0, 3, 6, 9], NOISE_LIST, TR05_SIMU_LIST, args.clean_type, True, "power_norm", args.data_augment)
    train_dataloader = FbankDataloader(train_dataset, opts, BATCH_SIZE, True, num_workers=4, drop_last=True)
    valid_dataset = FrameDataset([-6, -3, 0, 3, 6, 9], NOISE_LIST, DT05_SIMU_LIST, args.clean_type, False, "power_norm", "None")
    ...
    for epoch in range(args.total_epoch):
        if epoch > 0 or train_first:
            ...
            for iteration, (clean_frame, noisy_frame, frame_number, batch_mask) in enumerate(train_dataloader):
                ...
                # results["condition"] = log_feat
                results["condition"] = feat
                ...
                if args.dist_alpha > 0:
                    ...
                    rand_data = torch.rand_like(real_data) * 2. - 1.
                    rand_data[:, 1, :, :] = real_data[:, 1, :, :]
                    rand_logits, rand_feat = discriminator((rand_data.detach(),))

                    real_mean = real_logits.mean()
                    fake_mean = fake_logits.mean()

                    real_loss = ((real_logits - fake_mean) + 1.).pow(2.).mean()
                    fake_loss = ((fake_logits - real_mean) - 1.).pow(2.).mean()

                    # dist_loss = real_loss + fake_loss
                ...
                if global_step % args.summary_interval == 0:
                    ...
                    if args.dist_alpha > 0 or args.feat_alpha > 0:
                        add_tf_image(writer, real_data[:, 0, :, :], "train/real_data", 64, 8,
                                     global_step // args.summary_interval)
                        add_tf_image(writer, fake_data[:, 0, :, :], "train/fake_data", 64, 8,
                                     global_step // args.summary_interval)
