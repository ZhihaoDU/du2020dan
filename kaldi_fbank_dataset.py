...
class FrameDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        frame_list.append(self.enframe_speech_pair(clean_wav / c, noisy_wav / c))
        ...
        if self.data_augment != "None":
            ...
            if self.data_augment == "naive":
                ...
                frame_list.append(self.enframe_speech_pair(clean_wav / c, _noisy_speech / c))
...
class FbankDataloader(DataLoader):
    ...
    def my_collate_func(frames_list):
        ...
        item_len = len(frames_list[0])
        batch_clean = np.zeros([item_len * batch_size, max_frame, 400], np.float32)
        batch_noisy = np.zeros([item_len * batch_size, max_frame, 400], np.float32)
        batch_mask = np.zeros([item_len * batch_size, max_frame], np.float32)
        batch_frame_number = [0] * item_len * len(frames_list)
        for i, one_list in enumerate(frames_list):
            for j, data in enumerate(one_list):
                tt = min(one_list[j][2], max_frame)
                batch_clean[j * batch_size + i, :tt, :] = one_list[j][0][:tt, :]
                batch_noisy[j * batch_size + i, :tt, :] = one_list[j][1][:tt, :]
                batch_frame_number[j * batch_size + i] = tt
                batch_mask[j * batch_size + i, :tt] = 1.
            i += 1
