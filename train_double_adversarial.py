...
def save_check_point(generator, enhancer, discriminator, optim_g, optim_e, optim_d, global_step, model_name):
...
def load_check_point(generator, enhancer, discriminator, optim_g, optim_e, optim_d, global_step, model_name):
    ...
    enhancer.load_state_dict(check_point["enhancer"])
    optim_e.load_state_dict(check_point["optim_e"])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--adversarial_loss', type=str, default="lsgan_rgp")
    ...
    model_name_list = ["double", args.feature_domain, args.adversarial_loss, args.model_config]
    ...
    train_dataset = FrameDataset([-6, -3, 0, 3, 6, 9], NOISE_LIST, TR05_SIMU_LIST, args.clean_type, True,
    ...
    valid_dataset = FrameDataset([-6, -3, 0, 3, 6, 9], NOISE_LIST, DT05_SIMU_LIST, args.clean_type, False,
    ...
    Enhancer, Discriminator = get_model(gen_model, dis_model)
    ...
    enhancer = Enhancer(model_opts['gen_model_opts']).to(device)
    e_optimizer = optim.Adam(enhancer.parameters(), lr=2e-4, betas=(0.5, 0.999))
    ...
    if args.global_step > 0:
        load_check_point(generator, enhancer, discriminator, g_optimizer, e_optimizer, d_optimizer, args.global_step, model_name)
    ...
    for epoch in range(args.total_epoch):
        if epoch > 0 or train_first:
            generator.train()
            enhancer.train()
            # Disable the track_running_state of BatchNormalization in discriminator. Important!!
            discriminator.eval()
            ...
            for iteration, (clean_frame, noisy_frame, frame_number, batch_mask) in enumerate(train_dataloader):
                ...
                if args.target_type.lower() == "mapping_mag":
                    predict = enhancer.forward(feat.sqrt())
                else:
                    predict = enhancer.forward(log_feat)
                ...
                label_slices, real_data, enhanced_data = get_speech_slices(label, results, frame_number)

                z = torch.randn(real_data.size(0), 128, 1, 1).to(real_data)
                gen_data = generator.forward((z.detach(),))

                gan_bs = real_data.size(0)
                idx = np.arange(gan_bs)
                np.random.shuffle(idx)
                fake_data = torch.cat([enhanced_data[idx[:gan_bs//2]], gen_data[idx[gan_bs//2:]]], dim=0)
                ...
                if args.dist_alpha > 0:
                    ...
                    enhanced_logits, enhanced_feat = discriminator((enhanced_data,))
                    ...
                    e_loss = e_loss + dist_loss * args.dist_alpha
                ...
                if args.feat_alpha > 0:
                    ...
                    enhanced_logits, enhanced_feat = discriminator((enhanced_data,))
                    real_logits, real_feat = discriminator((real_data.detach(),))
                    feat_loss = (enhanced_feat - real_feat).pow(2.).mean()
                    e_loss = e_loss + feat_loss * args.feat_alpha

                recon_loss = reconstruction_loss(predict, target, batch_mask, backward=False, opts=None)[
                                 "loss"] / BATCH_SIZE
                ...
                if args.glc_alpha > 0:
                    if args.target_type.lower() == "mapping_mag":
                        ...
                        batch = log_feat
                    gp = calc_gradient_penalty(enhancer, batch, batch_mask)
                    (gp * args.glc_alpha).backward()
                e_optimizer.step()

                g_optimizer.zero_grad()
                disc_g_loss_dict = disc_g_loss(discriminator, (gen_data,))
                gen_loss = disc_g_loss_dict["g_loss"]
                gen_logits_mean = disc_g_loss_dict["logits_mean"]
                gen_loss.backward()
                ...
                if global_step % args.log_interval == 0:
                    ...
                    if args.dist_alpha > 0:
                        ...
                        writer.add_scalars("train", {"real_logits": real_logits.mean().item(),
                                                     "enhanced_logits": enhanced_logits.mean().item(),
                                                     "gen_logits": gen_logits_mean.item(),
                    ...
                    if args.feat_alpha > 0:
                        ...
                        writer.add_scalars("train", {"real_logits": real_logits.mean().item(),
                                           ...
                                           global_step // args.log_interval)
                    if "rgp" in args.adversarial_loss:
                        writer.add_scalar("train/gradient_norm", disc_d_loss_dict["gradient_norm"].item(),
                                          global_step // args.log_interval)
                    if "gp" in args.adversarial_loss:
                ...
                if global_step % args.summary_interval == 0:
                    ...
                    if args.dist_alpha > 0 or args.feat_alpha > 0:
                        ...
                        add_tf_image(writer, enhanced_data.squeeze(1), "train/enhanced_data", 64, 8,
                                     global_step // args.summary_interval)
                        add_tf_image(writer, gen_data.squeeze(1), "train/gen_data", 64, 8,
                                     global_step // args.summary_interval)
                        add_tf_image(writer, fake_data.squeeze(1), "train/fake_data", 11**2, 11,
                                     global_step // args.summary_interval)
                        writer.add_histogram("train/real_logits_hist", real_logits, global_step // args.summary_interval)
                        writer.add_histogram("train/fake_logits_hist", enhanced_logits, global_step // args.summary_interval)
        ...
        save_check_point(generator, enhancer, discriminator, g_optimizer, e_optimizer, d_optimizer, global_step, model_name)
        ...
        with torch.no_grad():
            ...
            for iteration, (clean_frame, noisy_frame, frame_number, batch_mask) in enumerate(valid_dataloader):
                ...
                if args.dist_alpha > 0:
                    label_slices, real_data, fake_data = get_speech_slices(label, results, frame_number)
                    # get discriminative loss for generator
                    enhanced_logits, enhanced_feat = discriminator((fake_data,))
        ...
        if valid_loss[0] < lowest_valid_loss:
            ...
            save_check_point(generator, enhancer, discriminator, g_optimizer, e_optimizer, d_optimizer, 0, model_name)
            logger.info("Done.")
            lowest_epoch = epoch
        else:
