...
def calc_rescale_c(_noisy_speech, rs_method="None"):
    c = 1.
    if rs_method.lower() == 'value_norm':
        c = 32768.
    elif rs_method.lower() == "power_norm":
        c = np.sqrt(np.mean(np.square(_noisy_speech)))
    elif rs_method.lower() == "max_norm":
...
def random_mix_speech_noise(clean_speech, noise, snr, noise_from, noise_to, is_norm=False):
    ...
    # to_mix_speech = np.array(clean_speech)/(2.**15)
    # to_mix_noise = np.array(noise[noise_from: noise_to])/(2.**15)
    ...
    if len(clean_speech) < (noise_to - noise_from):
        ...
        to_mix_noise = np.array(noise[noise_start: noise_start+len(clean_speech)])
    elif len(clean_speech) > (noise_to - noise_from):
        segs = len(clean_speech) // (noise_to - noise_from)
        to_mix_noise[:(segs-1)*noise_to] = np.repeat(np.array(noise[noise_from: noise_to]), segs)
        noise_start = np.random.randint(noise_from, noise_to - (len(clean_speech)-(segs-1)*(noise_to - noise_from)))
        to_mix_noise[(segs-1)*noise_to:] = np.array(noise[noise_start: noise_start+(len(clean_speech)-(segs-1)*(noise_to - noise_from))])
...
def ideal_abs_complex_ratio(noisy_speech, clean_speech, win_len, shift_len, delta_size):
    ...
    if delta_size > 0:
        abs_complex_irm = abs_complex_irm[:, delta_size:-delta_size]
...
def ideal_complex_ratio(noisy_speech, clean_speech, win_len, shift_len, delta_size=0, method='clip'):
    ...
    if delta_size > 0:
        complex_irm = complex_irm[:, delta_size:-delta_size]
    return complex_irm

def phase_sensitive_mask(noisy_speech, clean_speech, win_len, shift_len, delta_size=0, method='PSM'):
    noisy_spect = stft_extractor(noisy_speech, win_len, shift_len, 'hanning')
    clean_spect = stft_extractor(clean_speech, win_len, shift_len, 'hanning')
    theta = np.angle(clean_spect) - np.angle(noisy_spect)
    if method=='PSM':
        mask = np.abs(clean_spect)/np.abs(noisy_spect)*np.cos(theta)
    else:
        mask = np.abs(clean_spect)*np.cos(theta)
    if delta_size > 0:
        mask = mask[:, delta_size:-delta_size]
    return mask
...
def mag_synthesis_speech(noisy_speech, spect_mag, win_type, win_len, shift_len, frame_offset=0, syn_method='A&R'):
    ...
    if win_type == 'hanning':
        ...
        window = np.ones(win_len)
    to_ifft = np.zeros(next_power, dtype=np.complex64)
    clean_speech = np.zeros((frames-1)*shift_len+next_power, dtype=np.float32)
    window_sum = np.zeros((frames-1)*shift_len+next_power, dtype=np.float32)
    for i in range(frames):
        one_frame = noisy_speech[(i+frame_offset)*shift_len: (i+frame_offset)*shift_len+win_len]
        windowed_frame = np.multiply(one_frame, window)
        stft = np.fft.fft(windowed_frame, next_power)
        masked_mag = spect_mag[:, i]
        to_ifft[:next_power//2+1] = masked_mag * np.exp(1j * np.angle(stft[:next_power//2+1]))
        to_ifft[next_power//2+1:] = np.conj(to_ifft[next_power//2-1:0:-1])
        speech_seg = np.real(np.fft.ifft(to_ifft, next_power))

        if syn_method == 'A&R' or syn_method == 'ALLEN & RABINER':
            clean_speech[i*shift_len:i*shift_len+next_power] += speech_seg
            # window_sum[i*shift_len:i*shift_len+next_power] += window
        ...
        # if i > 0:
        #     clean_speech[i*shift_len: (i-1)*shift_len+win_len] *= 0.5
    # window_sum = np.where(window_sum < 1e-6, 1e-6, window_sum)
    # result_speech[frame_offset*shift_len: frame_offset*shift_len+len(clean_speech)] = clean_speech # / window_sum
    return clean_speech
