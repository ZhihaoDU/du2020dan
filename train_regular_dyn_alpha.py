    ...
    if isinstance(model, nn.Module):
        for name, para in model.named_parameters():
            if para.grad is not None:
                shadow_grads[name] = para.grad.clone().detach()
                shadow_grads["%s_norm" % name] = shadow_grads[name].view(-1).norm()
    return shadow_grads


def set_gradients(model, grads_list, alpha_list):
    rescaled_alphas = [var/alpha_list[0] for var in alpha_list]
    num = len(grads_list)
    if isinstance(model, nn.Module):
        for name, para in model.named_parameters():
            grad = 0.
            for k in range(num):
                grad = grad + ((grads_list[k][name] / grads_list[k]["%s_norm" % name]) *
                               grads_list[0]["%s_norm" % name] * rescaled_alphas[k])
            para.grad.copy_(grad.detach())


def draw_grad_norms(grads_list, step, scope="train"):
    plt.figure()
    plt.hold(True)
    for one_dict in grads_list:
        grad_norms = np.zeros(len(one_dict) // 2)
        ii = 0
        for key, value in one_dict.items():
            if "norm" in key:
                grad_norms[ii] = value.item()
                ii += 1
        plt.plot(np.arange(len(grad_norms)), grad_norms)
    plt.savefig("log/%s/grad_%s_%09d.png" % (model_name, scope, step))
...
if __name__ == '__main__':
    ...
    if not os.path.exists("log/%s" % model_name):
        os.system("mkdir -p log/%s" % model_name)
    ...
    for epoch in range(args.total_epoch):
        if epoch > 0 or train_first:
            ...
            for iteration, (clean_frame, noisy_frame, frame_number, batch_mask) in enumerate(train_dataloader):
                ...
                recon_grads = get_gradients(generator)
                grads_list = [recon_grads]

                # add l1/l2 regularization
                if args.l1_alpha > 0:
                    reg_loss = l1_penalty = l1_regularization(generator, "weight")
                    g_optimizer.zero_grad()
                    reg_loss.backward(retain_graph=True)
                    reg_grads = get_gradients(generator)
                    grads_list.append(reg_grads)

                if args.l2_alpha > 0:
                    reg_loss = l2_penalty = l2_regularization(generator, "weight")
                    g_optimizer.zero_grad()
                    reg_loss.backward(retain_graph=True)
                    reg_grads = get_gradients(generator)
                    grads_list.append(reg_grads)
                ...
                if args.glc_alpha > 0:
                    if args.target_type.lower() == "mapping_mag":
                        ...
                        batch = log_feat
                    reg_loss = gp = calc_gradient_penalty(generator, batch, batch_mask)
                    g_optimizer.zero_grad()
                    reg_loss.backward(retain_graph=True)
                    reg_grads = get_gradients(generator)
                    grads_list.append(reg_grads)
                ...
                if args.dist_alpha > 0:
                    ...
                    reg_loss = dist_loss = (fake_logits - real_logits).pow(2.).mean()
                    g_optimizer.zero_grad()
                    reg_loss.backward(retain_graph=True)
                    reg_grads = get_gradients(generator)
                    grads_list.append(reg_grads)
                ...
                if args.feat_alpha > 0:
                    ...
                    fake_logits, fake_feat = discriminator((fake_data,))
                    real_logits, real_feat = discriminator((real_data.detach(),))
                    reg_loss = feat_loss = (fake_feat - real_feat).pow(2.).mean()
                    g_optimizer.zero_grad()
                    reg_loss.backward(retain_graph=True)
                    reg_grads = get_gradients(generator)
                    grads_list.append(reg_grads)
                ...
                g_optimizer.zero_grad()
                set_gradients(generator, grads_list, grad_alpha)
                g_optimizer.step()
                ...
                if global_step % args.summary_interval == 0:
                    ...
                    draw_grad_norms(grads_list, global_step, "train")
                global_step += 1
